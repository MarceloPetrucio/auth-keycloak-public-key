...
export class ManualAuthenticationStrategy implements AuthenticationStrategy {
    ...
    public async authenticate(accessToken: string): Promise<KeycloakUserInfoResponse> {
        const token:any = jwt.decode(accessToken, { complete: true }); // For once, we'd like to have the header and not just the payload
...
// Article
// https://itnext.io/protecting-your-nestjs-api-with-keycloak-8236e0998233